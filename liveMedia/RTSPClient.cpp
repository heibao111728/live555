    ...
    do {
        if (session != NULL) 
        {
            ...
            while ((subsession = iter.next()) != NULL) 
            {
                ...
                if (subsession->rtpSource() != NULL)
                {
                    subsession->rtpSource()->enableRTCPReports() = True; // start sending RTCP "RR"s now
                }
            }
        }
        else 
        {
            ...
            scaleOK = True;
            if (speedParamsStr != NULL && !parseSpeedParam(speedParamsStr, subsession->speed()))
            {
                break;
            }
            speedOK = True;
...
Boolean RTSPClient::handleGET_PARAMETERResponse(char const* parameterName, 
    char*& resultValueString, char* resultValueStringEnd) 
{
    do {
        ...
        if (parameterName != NULL && parameterName[0] != '\0') 
        {
            ...
            if (parameterNameLen > 0 
                && _strncasecmp(resultValueString, parameterName, parameterNameLen) == 0) 
            {
...
void RTSPClient::connectionHandler1() 
{
    ...
    do {
        ...
        if (getsockopt(fInputSocketNum, SOL_SOCKET, SO_ERROR, (char*)&err, &len) < 0 
            || err != 0) 
        {
...
void RTSPClient::handleResponseBytes(int newBytesRead) 
{
    do {
        ...
        RequestRecord* request;
        if (newBytesRead > 0) 
        { // The "RTSP response was truncated" error
                    ...
                    while ((request = fRequestsAwaitingResponse.dequeue()) != NULL) 
                    {
                        ...
                        { // assumes that the CSeq counter will never wrap around
                          // We never received (and will never receive) a response for this handler, so delete it:
                            ...
                            {
                                envir() << "WARNING: The server did not respond to our \"" 
                                    << request->commandName() 
                                    << "\" request (CSeq: "
                                    << request->cseq() 
...
RTSPClient::RequestRecord::RequestRecord(unsigned cseq, char const* commandName, 
    responseHandler* handler, MediaSession* session, 
    MediaSubsession* subsession, u_int32_t booleanFlags,
    double start, double end, float scale, char const* contentStr)
    : fNext(NULL), fCSeq(cseq), fCommandName(commandName), 
    fSession(session), fSubsession(subsession), fBooleanFlags(booleanFlags),
    fStart(start), fEnd(end), fAbsStartTime(NULL), fAbsEndTime(NULL), 
    fScale(scale), fContentStr(strDup(contentStr)), fHandler(handler) 
{
}
...
RTSPClient::RequestRecord::RequestRecord(unsigned cseq, responseHandler* handler,
    ...
    : fNext(NULL), fCSeq(cseq), fCommandName("PLAY"), 
    fSession(session), fSubsession(subsession), fBooleanFlags(0),
    fStart(0.0f), fEnd(-1.0f), fAbsStartTime(strDup(absStartTime)), 
    fAbsEndTime(strDup(absEndTime)), fScale(scale),
...
::createNew(UsageEnvironment& env, 
    onRTSPClientCreationFunc* creationFunc, Port ourPort,
...
::HandlerServerForREGISTERCommand(UsageEnvironment& env, 
...
::createNewRTSPClient(char const* rtspURL, 
    int verbosityLevel, 
    char const* applicationName, 
    int socketNumToServer) 
{
    ...
    char const* /*proxyURLSuffix*/) 
{
    ...
    { // By default, we don't implement "DEREGISTER"
      // Create a new "RTSPClient" object, and call our 'creation function' with it:
        RTSPClient* newRTSPClient 
            = createNewRTSPClient(url, fVerbosityLevel, fApplicationName, socketToRemoteServer);
