...
static RTSPServer* createRTSPServer(Port port)
{
    ...
    {
        return RTSPServerWithREGISTERProxying::createNew(*env, port, 
            authDB, 
            authDBForREGISTER, 65, 
            streamRTPOverTCP, 
            verbosityLevel, 
            username, password);
    }
...
int main(int argc, char** argv)
{
    ...
    // (Such frames are unreasonably large; the back-end servers should really not be sending 
    // frames this large!)
    ...
    *env << "LIVE555 Proxy Server\n"
    ...
    // Check command-line arguments: optional parameters, 
    //then one or more rtsp:// URLs (of streams to be proxied):
    progName = argv[0];
    if (argc < 2) usage();
    while (argc > 1)
    {
        ...
        switch (opt[1])
        {
            ...
            {   // specify a username and password 
                //(to be used if the 'back end' (i.e., proxied) stream requires authentication)
    ...
    {
        usage(); // there must be at least one "rtsp://" URL at the end 
    }
  ...
  // and then with the alternative port number (8554):
    ...
    for (i = 1; i < argc; ++i) 
    {
        ...
        else 
        {
            ...
            sprintf(streamName, "proxyStream-%d", i); 
        }
